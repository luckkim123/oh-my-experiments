...
| **Evidence tags** `[FINDING]/[EVIDENCE:file:lines]/[CONFIDENCE:HIGH|MED|LOW]` | `skills/sciomc/SKILL.md` | OMX analysis output schema — enforces repo "추측 금지, 코드/데이터로 증명" (no guessing, prove with code/data) rule structurally. |
...
## 2. Version-resilience analysis (won't compatibility break when OMC updates?)
...
8. **DONE 2026-05-31** — **(post-v0.1) workspace-specialization wiki** — promote `registry/findings/` from a flat grep store to an OMC-wiki-style keyword-indexed knowledge layer (frontmatter category/confidence/sources/links + auto index.md/log.md + tag>title>content scoring, NO embeddings) so OMX compounds per-workspace. Whole new design unit (§9) — needs its own brainstorm + plan; NOT folded into #6.
...
Trigger: agent judges "work complete" OR user says "정리해줘" (clean it up). Never fires on its own without surfacing.
...
3. **Show + ask.** Present the diff to the user, require explicit approval (this is the "검토/허락" (review/permission) gate — non-negotiable).
...
- **(NEW — build #8) workspace-specialization wiki** — the user's "쓸수록 이 workspace에 특화되어 간다" (the more you use it, the more it specializes to this workspace) goal. The §10.2 `registry/findings/<slug>.md` is the *seed* (flat keyword+tag markdown store, grep-only, no embeddings) but is shallow. Promote it to an OMC-wiki-style layer (verified against `marketplaces/omc/src/hooks/wiki/`): per-page YAML frontmatter (`title`/`tags`/`category`∈{architecture,decision,pattern,debugging,environment,convention,...}/`confidence`∈{high,med,low}/`sources`/`links`), an auto-maintained `index.md` catalog + append-only `log.md`, and a **keyword search** (`tokenize` → tag>title>content scoring; CJK bigram so Korean queries hit) — explicitly **NO vector embeddings** (OMC's hard constraint; D1/D2 minimal-surface). This is what makes OMX compound per-workspace: exp-init seeds the workspace's structure/conventions, every exp-analyze/exp-design/exp-loop run appends category-tagged findings, and later runs query the accumulated knowledge. **Scope guard:** this is a whole new design unit (OMC implements it across ~8 files: storage/ingest/query/lint/session-hooks/types) — it does NOT belong in #6 exp-loop (which only appends to the existing flat store). Owed: a brainstorm + its own plan as **build #8** (after #7 finalizes the v0.1 4-skill set). Until then #6 appends to `registry/findings/` as-is. **[DONE 2026-05-31 — implemented as omx_core/wiki/ + omx wiki verbs; see HANDOFF.]**
```
```
