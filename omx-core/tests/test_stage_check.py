...
# --- disarmed (fix-round-3): stage_check stays registered NOWHERE ----------
...
def test_stage_check_is_still_implemented_but_not_registered():
    """Inverse of test_hook_registration.py's parity check, for exactly one
    handler. task 8 (run-completion-gate round) built `stage_check`, a Stop
    gate measured -- against the 17 real STAGE-declaring transcripts under
    ~/.claude/projects, not synthetic ones -- to block 8 of them, on
    sessions doing substantial genuine work. The user's decision was "ship
    it disarmed": keep the function and its tests (a future redesign should
    not start from nothing), but never let it fire live. See the DISARMED
    comment block above `stage_check` in hooks/handlers.py for the full
    reasoning (the measurement and the two structural reasons a wider verb
    list cannot fix) before re-registering it.

    This test exists so a re-registration is a DELIBERATE act -- someone
    reads that comment, decides it no longer applies, and edits both this
    test and the registration together -- rather than a "tidy up this
    orphan handler" PR that silently re-ships a gate already proven to trap
    real sessions."""
